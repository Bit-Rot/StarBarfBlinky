...
void Adafruit_NeoPixel::setPixelColorWithBrightness(
...
uint8_t Adafruit_NeoPixel::getRed(uint32_t color) {
    uint8_t *p = (uint8_t*)&color;
    return p[rOffset];
}

uint8_t Adafruit_NeoPixel::getGreen(uint32_t color) {
    uint8_t *p = (uint8_t*)&color;
    return p[gOffset];
}

uint8_t Adafruit_NeoPixel::getBlue(uint32_t color) {
    uint8_t *p = (uint8_t*)&color;
    return p[bOffset];
}

/*
void Adafruit_NeoPixel::getGreen(uint16_t pos) {
    if(wOffset == rOffset) { // Is an RGB-type strip
      p = &pixels[n * 3];    // 3 bytes per pixel
    } else {                 // Is a WRGB-type strip
      p = &pixels[n * 4];    // 4 bytes per pixel
...
void Adafruit_NeoPixel::getBlue(uint16_t pos) {
    if(wOffset == rOffset) { // Is an RGB-type strip
      p = &pixels[n * 3];    // 3 bytes per pixel
    } else {                 // Is a WRGB-type strip
      p = &pixels[n * 4];    // 4 bytes per pixel
...
void Adafruit_NeoPixel::setPixelColorWithBrightness(
 uint16_t n, uint32_t c, uint8_t brightness) {
  if(n < numLEDs) {
    uint8_t 
      r = (uint8_t)(c >> 16) * brightness / 255,
      g = (uint8_t)(c >>  8) * brightness / 255,
      b = (uint8_t)c * brightness / 255;
    this->setPixelColor(n, Adafruit_NeoPixel::Color(r, g, b));
